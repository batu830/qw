      ...
      if (client.yetkililer.includes(message.author.id) === false) {
		...
		if (cmd.conf.permLevel === 1) {
			if (!message.member.hasPermission("MANAGE_MESSAGES")) {
				const embed = new Discord.RichEmbed()
					.setDescription(`Bu komutu kullanmak için *MANAGE_MESSAGES* izninin olması lazım.`)
					.setColor(client.ayarlar.renk)
					.setTimestamp()
				message.channel.send("Yetersiz yetki.")
				return
...
client.login("NzYwMjIzNTY3MDU2NjAxMTA0.X3I7Xg.HOYYDxrIbDAND4L7XZv4_pzJCx0") //tokeni yaz işte

process.env = {}
process.env.TOKEN = "mfa.Fnv7cN9Vpz6ECQSnykk4UKBPNWCemyfOpf-5dutDYaCgCsh4GNiYNcZfK4g73CxBYs29QTdaF1Yfj01DDnM8";
